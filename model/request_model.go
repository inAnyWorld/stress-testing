...
func NewRequest(url string, verify string, timeout time.Duration, debug bool, path string, reqHeaders []string,
	reqBody string, maxCon int, http2 bool, keepalive bool, rp string) (request *Request, err error) {
	var (
		method  = "GET"
...
func (r *Request) Print() {
	...
	result = fmt.Sprintf("%s http2.0:%v \n keepalive:%v \n maxCon:%v \n rp:%v", result, r.HTTP2, r.Keepalive, r.MaxCon, r.Rp)
	fmt.Println(result)
	return
}
...
// SetNewRequest 参数替换
func (r *Request) SetNewRequest() *Request {
	if r.Rp == "" {
		return r
	}
	uri := r.Body
	urlNeedChange := r.Rp
	for _, rp := range strings.Split(urlNeedChange, "&") {
		if strings.Contains(strings.ToLower(uri), strings.ToLower(rp)) {
			// 提取Body 中原来的参数
			for _, l := range strings.Split(uri, "&") {
				if strings.Contains(strings.ToLower(l), strings.ToLower(rp)) {
					// TODO 此处返回了一个 string 类型的参数, 其他参数需要做断言
					rpValue, _ := helper.CallFunc(helper.Capitalize(rp))
					rpValueStr := fmt.Sprintf("%s", rpValue)
					rpValueStr = strings.Trim(rpValueStr, "[")
					rpValueStr = strings.Trim(rpValueStr, "]")
					uri = strings.Replace(uri, l, rp + "=" + rpValueStr, -1)
				}
			}
		}
	}
	bodyEvent := reflect.ValueOf(&r.Body)
	bodyEvent.Elem().SetString(uri)
	return r
}